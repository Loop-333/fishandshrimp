...
export default function FishCard({ fish, onViewDetails }) {
  ...
  // Get first PNG image
  const mainImage = fish.imageUrl?.split(';').find((img) => img.trim().endsWith('.png')) || '';

  // Get first GIF image (for hover)
  const gifImage = fish.imageUrl?.split(';').find((img) => img.trim().endsWith('.gif'));
    ...
    >
      ...
      <div className="mt-4 space-y-2">
        ...
        <button
          ...
          className={`mt-2 px-4 py-2 w-full rounded text-white transform transition-all duration-150 active:scale-95 active:brightness-90 ${
            fish.stock === 0
